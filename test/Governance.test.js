...
contract("Governance", function([
  ...
  describe("ProxyAdmin multisig tx", function() {
          ...
          it("reverts", async function() {
            await shouldFail.reverting(
              this.proxyAdmin.upgrade(
                tokenProxy.address,
                this.tokenImplementationV2.address,
                { from: anotherAccount }
              )
            );
            await shouldFail.reverting(
              this.proxyAdmin.upgrade(
                walletProxy.address,
                this.walletImplementationV2.address,
                { from: deployer }
              )
            );
          });
          ...
          it("reverts", async function() {
            await shouldFail.reverting(
              this.wallet.submitTransaction(
                proxyAdmin.address,
                0,
                "0x99a88ec4000000000000000000000000" +
                  tokenProxy.address.replace("0x", "") +
                  "000000000000000000000000" +
                  this.tokenImplementationV2.address.replace("0x", ""),
                { from: anotherAccount }
              )
            );
            await shouldFail.reverting(
              this.wallet.submitTransaction(
                ...
                0,
                "0x99a88ec4000000000000000000000000" +
                  walletProxy.address.replace("0x", "") +
                  "000000000000000000000000" +
                  this.walletImplementationV2.address.replace("0x", ""),
                { from: anotherAccount }
              )
            );
          });
        ...
        describe("upgrade", function() {
          ...
          it("submitted but not executed and does not require finalization", async function() {
            (await this.proxyAdmin.getProxyImplementation(
              tokenProxy.address
            )).should.equal(tokenImplementation.address);
            (await this.wallet.requireFinalization(0)).should.equal(true);

            (await this.proxyAdmin.getProxyImplementation(
              walletProxy.address
            )).should.equal(walletImplementation.address);
            (await this.wallet.requireFinalization(1)).should.equal(true);
          });
          ...
          it("approved by other admin but requireFinalization", async function() {
            ...
            await this.wallet.confirmTransaction(1, { from: account_2 });
            (await this.proxyAdmin.getProxyImplementation(
              walletProxy.address
            )).should.equal(walletImplementation.address);
          });
          ...
          it("execute after owner finalization", async function() {
            ...
            (await this.proxyAdmin.getProxyImplementation(
              tokenProxy.address
            )).should.equal(this.tokenImplementationV2.address);

            await this.wallet.confirmTransaction(1, { from: account_2 });
            await this.wallet.finalizeTransaction(1, { from: deployer });
            (await this.proxyAdmin.getProxyImplementation(
              walletProxy.address
            )).should.equal(this.walletImplementationV2.address);
          });
          ...
          it("cannot re-execute after owner finalization", async function() {
            ...
            await this.wallet.finalizeTransaction(1, { from: deployer });
            await shouldFail.reverting(
              this.wallet.finalizeTransaction(1, { from: deployer })
            );
          });
        });
      });
    ...
    describe("upgradeandcall", function() {
          ...
          it("reverts", async function() {
            await shouldFail.reverting(
              this.wallet.submitTransaction(
                proxyAdmin.address,
                0,
                "0x9623609d000000000000000000000000" +
                  tokenProxy.address.replace("0x", "") +
                  "000000000000000000000000" +
                  this.tokenImplementationV2.address.replace("0x", "") +
      ...
      describe("called by an admin account", function() {
        describe("upgradeandcall wallet", function() {
          beforeEach("submit transaction", async function() {
            await this.wallet.submitTransaction(
              proxyAdmin.address,
              0,
        ...
        /*describe('upgradeandcall token', function (){
          ...
          it('submitted but not executed and does not require finalization', async function () {
            (await this.proxyAdmin.getProxyImplementation(tokenProxy.address)).should.equal(tokenImplementation.address);
            (await this.wallet.requireFinalization(0)).should.equal(true);
          });
          ...
          it('execute after owner finalization', async function () {
            ...
            (await this.proxyAdmin.getProxyImplementation(tokenProxy.address)).should.equal(this.tokenImplementationV2.address);
          });

          it('cannot re-execute after owner finalization', async function () {
            await this.wallet.confirmTransaction(0, { from : account_2 });
            await this.wallet.finalizeTransaction(0, { from : deployer });
            await shouldFail.reverting(this.wallet.finalizeTransaction(0, { from : deployer }));
          });
        });*/
      });
    });
